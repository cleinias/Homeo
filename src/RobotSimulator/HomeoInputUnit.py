...
class HomeoTCPInputUnit(HomeoUnit):
    '''
    Class HomeoTCPInputUnit is an interface to an external input device connected through a TCP server that the robot runs/
    Instead of computing its value from the other units, HomeoTCPInputUnit
    reads it from a (simulated) robot's sensor. 
    It does so by reimplementing the selfUpdate method to send a message using the Khepera protocol (slightly modified) that the robot server 
    runs.

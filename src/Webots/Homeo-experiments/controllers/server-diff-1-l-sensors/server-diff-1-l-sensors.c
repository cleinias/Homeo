...
//    printf("Received %d bytes: %s\n", n, buffer);
  ...
  /* The controller simulates a one motor robot by always sending 
     the same speed command to both wheels at once.
     Thus, the L and R commands have actually the same effect */
...
//        double left_speed = wb_differential_wheels_get_left_speed();
        wb_differential_wheels_set_speed(right_speed, right_speed);
        send(fd, "d\r\n", 3, 0);
